...
function renderPublishedAwardsCard() {
  ...
  sectionsEl.innerHTML = PUBLISHED_AWARD_SECTIONS.map((section) => {
...
function normalizePublishedSectionName(value) {
  ...
  if (!text) return "Special Awards/Scholarships";
  if (text.includes("solo")) return "Solos";
  if (text.includes("duo") || text.includes("trio")) return "Duo/Trios";
  if (text.includes("group")) return "Groups";
